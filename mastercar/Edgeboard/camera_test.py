...
frame=[None,None]
while True:
	ok1, frame[0] = cap1.read()  # 读取每一帧图片
	ok2, frame[1] = cap2.read()  # 读取每一帧图片
	# 图形基本转换
	for i in range(0,2):
		frame[i] = cv2.GaussianBlur(frame[i], (5, 5), 0)  # 高斯模糊
		hsv = cv2.cvtColor(frame[i], cv2.COLOR_BGR2HSV)  # 转换 hsv
		mask = cv2.inRange(hsv, red_min, red_max)  # 生成掩膜
		# 形态学操作
		mask = cv2.erode(mask, None, iterations=2)
		mask = cv2.dilate(mask, None, iterations=2)
		mask = cv2.GaussianBlur(mask, (3, 3), 0)
		res = cv2.bitwise_and(frame[i], frame[i], mask=mask)  # 与运算
		...
		if len(cnts) > 0:
			...
			cv2.circle(frame[i], (int(x), int(y)), int(radius) * 2,

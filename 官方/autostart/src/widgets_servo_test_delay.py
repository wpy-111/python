...
class Servo:
    ...
    def servocontrol(self, angle, speed):
        cmd_servo_data = bytes.fromhex('77 68 06 00 02 36') + bytes.fromhex(self.ID_str) + speed.to_bytes(1,byteorder='big', signed=True)\
...
class Servo_pwm:
    ...
    def servocontrol(self, angle, speed):
        cmd_servo_data = bytes.fromhex('77 68 06 00 02 0B') + bytes.fromhex(self.ID_str) + speed.to_bytes(1,byteorder='big', signed=True) \
...
if __name__ == '__main__':
    ...
    # 相机线程拉起，注意连接两个usb摄像头到EdgeBoard板子上
